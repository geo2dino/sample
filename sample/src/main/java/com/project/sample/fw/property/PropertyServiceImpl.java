package com.project.sample.fw.property;
...
public class PropertyServiceImpl
	implements PropertyService, InitializingBean, DisposableBean, ResourceLoaderAware {
...
//			LOGGER.error(messageSource.getMessage("error.properties.refresh.files.reason", new String[] {}, Locale.getDefault()));
			throw new Exception("error.properties.refresh.files" + new String[] { fileName }, e);
...
//		LOGGER.debug(messageSource.getMessage("debug.properties.filename", new String[] { resource.getFilename(), encoding }, Locale.getDefault()));
		ExtendedProperties extendedProperty = new ExtendedProperties();
		extendedProperty.load(resource.getInputStream(), encoding);
		extendedProperties.combine(extendedProperty);
	}
}
